            ...
            <div class="p-6 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                <header class="mb-4 flex justify-between items-center">
                    <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100">
                        {{ __('Daftar Prioritas Kebutuhan') }}
                    </h2>
                    <!-- Tombol Tambah -->
                    <a href="{{ route('prioritas-kebutuhan.create') }}"
                       class="px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700 transition">
                        Tambah Prioritas Kebutuhan
                    </a>
                </header>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full border border-gray-300 dark:border-gray-700 rounded-lg">
                        ...
                        <tbody class="text-gray-900 dark:text-gray-100">
                            @foreach($prioritasKebutuhan as $item)
                            <tr class="border border-gray-300 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-800 transition">
                                ...
                                <td class="px-4 py-2 border border-gray-300 dark:border-gray-700">{{ $item->prioritas }}</td>
                                ...
                                <td class="px-4 py-2 border border-gray-300 dark:border-gray-700">
                                    <div class="flex items-center justify-center space-x-2">
                                        <!-- Tombol Edit -->
                                        <a href="{{ route('prioritas-kebutuhan.edit', $item->id) }}"
                                        ...
                                        <form action="{{ route('prioritas-kebutuhan.toggle-status', $item->id) }}" method="POST">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit"
